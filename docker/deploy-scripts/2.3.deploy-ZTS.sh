...
docker exec --user mysql:mysql \
  ...
  --execute="CREATE USER 'zts_admin'@'${ZTS_HOST}.${DOCKER_NETWORK}' IDENTIFIED BY '${ZTS_DB_ADMIN_PASS}'; GRANT ALL PRIVILEGES ON zts_server.* TO 'zts_admin'@'${ZTS_HOST}.${DOCKER_NETWORK}'; FLUSH PRIVILEGES;"
docker exec --user mysql:mysql \
  "${ZTS_DB_HOST}" mysql \
  --database=mysql \
  --user=root --password="${ZTS_DB_ROOT_PASS}" \
