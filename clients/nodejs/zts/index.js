...
class ZTSClient {
  constructor(params) {
   ...
   static _initConfigValues() {
    ...
    if (config.zts_client_token_min_expiry_time >= 0) {
      tokenMinExpiryTime = config.zts_client_token_min_expiry_time;
    } else {
      tokenMinExpiryTime = 900;
    }

    if (config.zts_client_disable_cache) {
      cacheDisabled = config.zts_client_disable_cache;
    } else {
      cacheDisabled = false;
    }
  ...
  _sameCredentialsAsBefore(svcPrincipal) {
    ...
    var creds = this.principal.getCredentials();
    if (!creds) {
      return false;
    }
  ...
  _updateServicePrincipal() {
    ...
    var svcPrincipal = this._siaProvider.getIdentity(this._domain, this._service);
    ...
    if (!svcPrincipal) {
      var msg = 'UpdateServicePrincipal: Unable to get PrincipalToken ' +
  ...
  getRoleToken(params, cb) {
    ...
    if (!cacheDisabled) {
      ...
      if (cacheKey && !params.ignoreCache) {
        roleToken = this._lookupRoleTokenInCache(cacheKey);
        if (roleToken) {
  ...
  _lookupRoleTokenInCache(cacheKey) {
    var roleToken = cache.get(cacheKey);
    if (!roleToken) {
    ...
    var expiryTime = roleToken.getExpiryTime() - Math.floor(Date.now() / 1000);
    if (this._isExpiredToken(expiryTime, this.minExpiryTime, this.maxExpiryTime, tokenMinExpiryTime)) {
